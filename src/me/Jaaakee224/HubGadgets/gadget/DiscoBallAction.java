...
public class DiscoBallAction implements Gadget.GadgetAction {
	...
	public void onFinish(final Player player, final Gadget gadget) {
		...
		if (blocks != null) {
			for (final Block block : blocks) {
				block.getWorld().playEffect(block.getLocation(), Effect.RECORD_PLAY, 0);
	...
	public boolean onRun(final Player player, final long ticks, final Gadget gadget) {
		if (ticks % 20L == 0L) {
			...
			ParticleEffect.FIREWORKS_SPARK.display(effect, 0.0f, 0.0f, 0.0f, 0.5f, 30);
			effect.getBlock().setData((byte) MathUtils.random.nextInt(15));
