    ...
    public boolean onRun(final Player player, final long ticks, final Gadget gadget) {
        if (ticks % 4L == 0L) {
            ...
            for (int i = 0; i < 3; ++i) {
                ...
                final Item item = world.dropItem(location, ItemBuilder.getRandomResource(Material.WOOL, (short)MathUtils.random.nextInt(16)));
                item.setPickupDelay(Integer.MAX_VALUE);
                item.setVelocity(new Vector(MathUtils.random(-0.2f, 0.2f), 0.4f, MathUtils.random(-0.2f, 0.2f)));
                world.playSound(player.getLocation(), Sound.CHICKEN_EGG_POP, 10, 1);
                entities.add(item);
            }
