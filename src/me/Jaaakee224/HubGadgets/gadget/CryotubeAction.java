...
public class CryotubeAction implements Gadget.GadgetAction
{
    ...
    public boolean onEvent(final Event event, final Gadget.TriggerAction trigger, final Gadget gadget) {
        ...
        if (trigger == Gadget.TriggerAction.INTERACT) {
            ...
            this.addBlock(location.getBlock(), blocks).setType(Material.ICE);
            this.addBlock(location.add(0.0, 1.0, 0.0).getBlock(), blocks).setType(Material.ICE);
            this.addBlock(location.add(0.0, 1.0, 0.0).getBlock(), blocks).setType(Material.STEP);
